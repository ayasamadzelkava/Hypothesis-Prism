...
// -------------------------------------
// Unified Collapse Rendering on IDgraph Based on Internal Levels
// -------------------------------------
/*
  In the unified IDgraph tree:
   - I-tree nodes use their declared level as their internal level.
   - D-tree nodes are remapped with: internal level = declared level + (n - 1),
     where n = (max(I-tree) + 1).

  With this mapping, if I-tree has levels 0..(n-1) and D-tree has declared levels 0..(m-1),
  then the unified internal levels become:
      0,1,..., n-1, n, ..., n+m-2.
  The total number of levels is n+m-1.

  This function traverses the unified graph (applied separately to the I and D parts)
  and performs collapse based on a global collapse set. If a node’s internal level is
  in the collapse set, its marker and label are not drawn and its children are attached
  directly to the last visible (effective) parent so that a bridging edge is drawn.

  It returns a Pair:
    - first: the rendered Group for that subtree
    - second: a list of visible leaves (for drawing cross edges)
*/
fun renderCollapsedIDGraph(
    node: TreeNode,
    effectiveParent: TreeNode?,
    ...
    val group = Group()
    val visibleLeaves = mutableListOf<TreeNode>()
    val internalLevel = getInternalLevel(node)
    val isCollapsed = internalLevel in collapseLevels

    if (!isCollapsed) {
        group.children.add(createVertexMarker(node.position, nodeRadius, nodeColor))
        group.children.add(createTextLabel(node.label, node.position, Color.BLACK))
    }
    // If collapsed, do NOT update the effective parent with this node.
    val newEffective = if (!isCollapsed) node else effectiveParent
...
class GraphTetrahedronApp : Application() {
    ...
    // Global collapse set: internal levels in this set will be collapsed.
    private val globalCollapseLevels: MutableSet<Int> = mutableSetOf()

    // Group holding the rendered unified IDgraph tree (collapsed) and cross edges.
    private var idGraphCollapseGroup: Group = Group()
    ...
    private fun refreshTrees() {
        sceneGroup.children.remove(idGraphCollapseGroup)
        // For I tree: internal mapping = identity.
        val (iTreeRender, iVisibleLeaves) = renderCollapsedIDGraph(iTreeRoot, null, globalCollapseLevels, { it.level }, Color.GREEN, 3.0)
        // For D tree: internal mapping = declared level + (n - 1), where n = max(I-tree)+1.
        val n = computeMaxLevel(iTreeRoot) + 1
        val m = computeMaxLevel(dTreeRoot) + 1
        val getInternalD: (TreeNode) -> Int = { node -> node.level + (n - 1) }
        val (dTreeRender, dVisibleLeaves) = renderCollapsedIDGraph(dTreeRoot, null, globalCollapseLevels, getInternalD, Color.BLUE, 3.0)
        // Build cross edges (purple) between every visible leaf of I tree and D tree.
        val crossEdges = Group()
        for (iLeaf in iVisibleLeaves) {
    ...
    // Update the toggle overlay: one column of toggle arrows for all distinct internal levels.
    private fun updateToggleOverlay() {
        toggleOverlayGroup.children.clear()
        val iMax = computeMaxLevel(iTreeRoot) // I-tree declared levels 0..iMax.
        val n = iMax + 1
        val m = computeMaxLevel(dTreeRoot) + 1
        // For D tree, internal levels range from (n-1) to (n-1)+(m-1) = n+m-2.
        val dLevels = (n - 1..n + m - 2).toList()
        val unionLevels = (0..iMax).toList() + dLevels
        val sortedLevels = unionLevels.sorted()
        var i = 0
        for (lvl in sortedLevels) {
    ...
    override fun start(primaryStage: Stage) {
        // Define tetrahedron vertices (for positioning the trees on different faces).
        val iSource = Point3D(0.0, 100.0, 0.0)
        val dSource = Point3D(-100.0, -100.0, 100.0)
        val dLeftEndpoint = Point3D(100.0, -100.0, 100.0)
        val dRightEndpoint = Point3D(0.0, -100.0, -100.0)
        ...
        primaryStage.title = "Unified IDgraph Tree with Continuous Levels and Collapse"
        primaryStage.scene = scene
        primaryStage.show()
    }
}
